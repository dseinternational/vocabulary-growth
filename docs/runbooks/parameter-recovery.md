...
One scored quantity is **derived rather than read from the graph**: the total spread, the SD in words of one administration of one new child at each query age (`total_spread_words_query` on the single-outcome models, `total_spread_words_u_query` and `total_spread_words_s_query` on the bivariate ones). It is the published Down syndrome / typically developing between-child contrast ([#229](https://github.com/dseinternational/vocabulary-growth/issues/229) option 4), and [#289](https://github.com/dseinternational/vocabulary-growth/issues/289) task 4.12 made it a recovery target because nothing had ever scored it. `compare_replicate` computes it from the probability-scale curves, the concentrations and the child scales, on the posterior and on the truth draw alike, by the same function `scripts/compare_ds_td_re.py` publishes it with (`comparison.total_spread_from_values`). It is not derived for the joint engine or VG22's low-rank factor, which keep their previous target set, nor in a cross-definition run whose truth sits on a different query grid, where no age describes the same child on both sides.

It is a function of quantities already scored (the probability curves, the concentrations and the child scales), as `p_s_query` is of `p_u_query` and `q_query`. Its rows are therefore correlated with theirs, and they add 14 rows per outcome for VG20, so the replicate's `coverage_ci89` leans further on the trajectory block. Read the total-spread rows on their own for the #229 condition. Adding them changes the target count too, so a matrix scored before 2026-09-14 is not comparable row for row with one scored after. See [`notes/202609141600-total-spread-estimand.md`](../../notes/202609141600-total-spread-estimand.md).
