...
Section~\ref{sec-findings-signing}) rather than against a fixed cut-off.
...
throughout this report. See Section~\ref{sec-findings-signing} for its
scope and caveats.
...
\section{How many words at each age}\label{sec-findings-ds}

Children with Down syndrome understand far more words than they say, and
both grow steadily across early and middle childhood. Under the model of
record (VG10), a typical child is estimated to understand about ---
words at 24 months, --- at 36 months, --- at 60 months, and --- by 90
months (out of the 800-word reference inventory;
Section~\ref{sec-betabinomial}). Spoken vocabulary lags well behind:
about --- words at 24 months, --- at 36 months, and --- at 60 months.

Figure~\ref{fig-ds-joint} shows both trajectories with their uncertainty
bands. The estimates here are for the \emph{typical} child (posterior
median with a 94\% credible interval); the wider bands for individual
children are tabulated in \textbf{?@sec-reference-tables}.

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG10-age-understood-spoken-ds-re-subj-uq-anchored/joint_trajectory_hdi.png}}

\caption{\label{fig-ds-joint}Expected words understood and spoken by
age, children with Down syndrome (VG10), with 50\% and 94\%
...
The full age-by-age tables --- expected counts for a typical child
\emph{and} the range across individual children --- are in
\textbf{?@sec-reference-tables}. The univariate models VG01 (spoken) and
VG02 (understood) give closely agreeing cross-checks (Appendix B1).

\end{tcolorbox}

\section{How fast children learn}\label{how-fast-children-learn}

The rate of word learning is not constant: it accelerates through the
preschool years as vocabulary builds. Figure~\ref{fig-ds-rate-u} and
Figure~\ref{fig-ds-rate-s} show the expected number of new words gained
per month for understanding and speaking. Comprehension growth peaks
earlier; spoken-word growth accelerates later, once production gets
underway (see the production ratio below).

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG10-age-understood-spoken-ds-re-subj-uq-anchored/expected_learning_rate_u.png}}

\caption{\label{fig-ds-rate-u}Expected rate of learning words understood
(words/month) by age (VG10).}

\end{figure}%

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG10-age-understood-spoken-ds-re-subj-uq-anchored/expected_learning_rate_s.png}}

\caption{\label{fig-ds-rate-s}Expected rate of learning words spoken
(words/month) by age (VG10).}

\end{figure}%

\section{Understanding versus saying: the comprehension--production
gap}\label{understanding-versus-saying-the-comprehensionproduction-gap}

A central feature of vocabulary development in Down syndrome is that
understanding runs ahead of production. The \textbf{production ratio}
\(q(a) = P(\text{speak} \mid \text{understood})\)
(Section~\ref{sec-ratios}) captures the fraction of understood words a
child also says. Under VG10 it rises from about --- at 24 months to
about --- at 48 months and around --- by 72 months, after which it
plateaus --- children come to say most, but not all, of the words they
understand.

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG10-age-understood-spoken-ds-re-subj-uq-anchored/production_rate.png}}

\caption{\label{fig-ds-q}The production ratio \(q(a)\) --- the fraction
of understood words that are spoken --- by age (VG10).}

\end{figure}%

Expressed as counts, the \textbf{gap} between words understood and words
spoken widens through the preschool years --- reaching roughly --- words
around 37 months --- and then narrows as production catches up
(Figure~\ref{fig-ds-gap}).

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG10-age-understood-spoken-ds-re-subj-uq-anchored/comprehension_production_gap.png}}

\caption{\label{fig-ds-gap}The comprehension--production gap (expected
words understood minus spoken) by age (VG10).}

\end{figure}%

\section{How much children differ}\label{how-much-children-differ}

Variability between children is large and grows with age. The model
captures this in two ways: age-varying dispersion \(\kappa(z)\)
(Section~\ref{sec-kappa}), which widens the spread of counts at older
ages, and subject random effects (Section~\ref{sec-randomeffects}),
which allow each child their own level. Figure~\ref{fig-ds-spread} shows
the resulting distribution of spoken-word counts by age --- the range
across individual children is much wider than the uncertainty in the
typical trajectory, which is why the practical reference
(\textbf{?@sec-reference-tables}) reports both.

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG10-age-understood-spoken-ds-re-subj-uq-anchored/posterior_predictive_count_distributions_s.png}}

\caption{\label{fig-ds-spread}Posterior predictive distribution of words
spoken by age, children with Down syndrome (VG10).}

\end{figure}%

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-warning-color!10!white, colframe=quarto-callout-warning-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Scope and caveats}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

\textbf{TODO}: the shared DS findings caveats, stated once ---
parent-report measures; cross-study harmonisation onto the 800-item
reference (Section~\ref{sec-data}); the sample is not a random sample of
all children with Down syndrome (representativeness); estimates describe
the modelled population and are conditional on the model. Numbers are
drawn from the model of record (VG10) at the canonical 6-monthly ages;
see Appendix C for the complete tables.
...
\section{The sign-given-understood trajectory
r(a)}\label{sec-findings-signing}

Signing plays a distinctive, time-limited role in early expressive
development. The \textbf{sign ratio}
\(r(a) = P(\text{sign} \mid \text{understood})\)
(Section~\ref{sec-ratios}) follows a rise-then-fall hump: under VG15 it
climbs from about --- of understood words at 18 months to a peak of
roughly --- around 36 months, then recedes to about --- by 60 months and
--- by 84 months. Figure~\ref{fig-sign-rate} shows the signed and spoken
ratios together, with study composition absorbed by the random
intercepts (Section~\ref{sec-randomeffects}) --- the signed peak that
was study-confounded in VG14 is now identified.

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG15-age-joint-signspeech-ds/signed_vs_spoken_rate.png}}

\caption{\label{fig-sign-rate}Signed and spoken production ratios by
age, children with Down syndrome (VG15).}

\end{figure}%

\section{Words migrating from sign to
speech}\label{words-migrating-from-sign-to-speech}

The hump reflects words moving \emph{through} sign into speech.
Figure~\ref{fig-sign-composition} shows the four mutually exclusive
states of an understood word --- sign-only, both signed and spoken,
speak-only, and neither --- as fractions of understood vocabulary over
age (Section~\ref{sec-association}). Early on, signed-and-not-yet-spoken
words make up a substantial share; as children develop, words migrate
into the spoken (and sign-plus-speech) categories.

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG15-age-joint-signspeech-ds/four_cell_composition.png}}

\caption{\label{fig-sign-composition}Within-understood four-cell
composition (sign-only / both / speak-only / neither) by age (VG15).}

\end{figure}%

\section{How sign and speech go together: the association
ψ}\label{how-sign-and-speech-go-together-the-association-ux3c8}

Within understood words, signing and speaking co-occur \textbf{more}
than chance: the Plackett odds ratio (Section~\ref{sec-association}) has
posterior median \(\psi \approx\) --- (94\% credible interval --------),
with posterior evidence to be inserted once the VG15 psi summary is
available. A word a child understands is more likely to be both signed
and spoken than two independent coin-flips at rates \(r\) and \(q\)
would predict. The association is a single descriptive scalar estimated
from a modest sample, so we lead with the interval and the strength of
evidence rather than the point value (see the caveat below).

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG15-age-joint-signspeech-ds/psi_posterior.png}}

\caption{\label{fig-sign-psi}Posterior of the sign--speech association
\(\psi\), with the independence reference \(\psi = 1\) (VG15).}

\end{figure}%

\section{Total expressive vocabulary}\label{total-expressive-vocabulary}

Combining speech and sign gives \textbf{total expressive vocabulary}
\(p_{\text{any}}\) (Section~\ref{sec-association}): the words a child
produces by either route. Because \(\psi > 1\) (signed and spoken words
overlap more than independence implies), the data-identified total sits
just \emph{below} the independence-based upper bound VG14 assumed ---
the gap is the double-counting VG14 could not avoid. Under VG15 a
typical child produces (by speech or sign) about --- words at 36 months
and --- at 60 months. Figure~\ref{fig-sign-pany} shows the identified
total against the independence bound.

\begin{figure}

\pandocbounded{\includegraphics[keepaspectratio]{figures/VG15-age-joint-signspeech-ds/p_any_identified_vs_bound.png}}

\caption{\label{fig-sign-pany}Data-identified total expressive
vocabulary \(p_{\text{any}}\) against VG14's independence upper bound
(VG15).}

\end{figure}%

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-important-color!10!white, colframe=quarto-callout-important-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{The association is descriptive and exploratory}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

The within-understood association \(\psi\) (and therefore the
data-identified \(p_{\text{any}}\)) rests on the \textasciitilde60
uk\_02 four-cell rows (\textasciitilde34 children, ≈19--56 months). It
is a single scalar --- the data do not support an age-varying
association at this sample size --- and is \textbf{descriptive /
exploratory}. The four-cell composition and \(p_{\text{any}}\) outside
that age window lean on the marginal trajectories plus scalar-\(\psi\)
extrapolation. Signing data exist only for the Down syndrome datasets,
so there is no typically-developing comparison.

At this sample size a point estimate that clears the independence
reference \(\psi = 1\) is, on average, inflated in magnitude --- the
Type-M (``winner's-curse'') consequence of selecting on a noisy quantity
--- so the credible interval and the strength of evidence, not the point
value of \(\psi\), should carry the interpretation.
