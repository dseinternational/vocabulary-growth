...
::: {.callout-note}
Drafted by an LLM-based AI tool (OpenAI Codex/GPT-5; random-effect scale priors by Claude Code/Opus 5).
:::
...
The trajectory priors match the TD spoken baseline anchors at 12 and 26 months. The dataset random-effect scale uses a HalfNormal(0.5) prior on the logit scale and the child one a HalfNormal(1.5), the latter calibrated against an estimate of 1.06 for this model (see `docs/models/PRIORS.md`). The 19-month GP anchor is a stabilisation constraint that separates the GP level from the intercept and random effects; it is not a substantive claim that development changes uniquely at 19 months.
