...
The parameter stays **descriptive/exploratory**. The four-cell composition below ~11 months, and $p_\text{any}$ there, still lean on the marginal trajectories plus $\psi$ extrapolation. `uk_07` is a randomised trial sample, though its association is present in both arms at baseline and is a property of its context rather than its trial; `include_uk07_cells=False` and `include_es01_cells=False` isolate each source's pull on $\psi$ while keeping its marginals in the fit. The primary analysis masks `uk_01` signing because it records sign-only rather than total signed words, which remains an explicit source sensitivity. `uk_06` signing is now included: the source confirmed the standard DSE checklists, whose column 2 ("understands and signs") is a total sign count, closing #211.
