...
::: {.callout-important title="psi is identified by two studies — descriptive / exploratory"}

The association $\psi$ is identified by the 138 rows carrying a four-cell cross-tab: 56 from `uk_02` (20–56 months) and 82 from `uk_07` (34–95 months). It remains a **single scalar** (the data do not support an age-varying association at this sample size) and **descriptive/exploratory**, but it no longer rests on one study or one age window — the two sources overlap between 34 and 56 months and between them span 20–95. The four-cell composition below ~20 months, and $p_\text{any}$ there, still lean on the marginal trajectories plus scalar-$\psi$ extrapolation. `uk_07` is a randomised trial sample, so its intervention arm's growth across the three assessment points is partly programme-driven; `include_uk07_cells=False` isolates its pull on $\psi$ while keeping its marginals in the fit. The primary analysis masks `uk_01` signing because it records sign-only rather than total signed words, and masks `uk_06` signing pending source verification; both are explicit source sensitivities.
