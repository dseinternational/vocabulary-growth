...
Dispersion follows `kappa(age) = kappa_min + exp(a_kappa + b_kappa * z)`, with the age term given priors at 18 and 36 months rather than an intercept at the pool's mean age and a slope. `a_kappa` and `b_kappa` are derived from those two anchors, so both priors are directly comparable with a per-age dispersion estimate and neither shifts meaning if the pool's age distribution changes. See `docs/models/PRIORS.md`.
...
For this baseline model, focus on the slope anchors, GP hyperparameters, and age-varying `kappa` parameters. Poor diagnostics there would directly affect the spoken trajectory and its cross-sectional age derivative.

:::

## Posterior predictions

The main findings to inspect are the spoken vocabulary trajectory, the age at which its cross-sectional derivative is largest, and how much uncertainty remains at older ages. Because this model has no study or subject effects, treat it as a baseline rather than the final DS spoken-vocabulary account.
