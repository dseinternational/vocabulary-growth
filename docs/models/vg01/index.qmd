...
### Expected age to reach vocabulary milestones

Using the posterior distribution of the latent mean trajectory, we estimate the age at which the expected number of words spoken first reaches key milestones (50, 100, 200, and 400 words). These distributions reflect uncertainty in the population-average trajectory, not individual-level variability.

```{python}
milestone_summary = pd.read_csv("milestone_age_summary.csv")
milestone_summary
```

![milestone_age_distributions](milestone_age_distributions.svg){#fig-milestone-ages .lightbox fig-align="left"}
