...
**Does receptive vocabulary predict expressive vocabulary, cross-sectionally?** The joint model links the two structurally through the production ratio $q(a) = P(\text{speak} \mid \text{understood})$: the expected number of words a child *says* is the number they *understand* scaled by $q(a)$,

$$\mathbb{E}[\,\text{spoken} \mid \text{understood} = N,\ \text{age} = a\,] \approx N \cdot q(a).$$

So receptive vocabulary predicts expressive vocabulary with an age-dependent slope $q(a)$ — the expected number of words spoken per word understood. The figure below reads this off directly: for typically-developing children across the 8–18 month window, it maps words understood (x-axis) to the predicted words spoken (y-axis), with the shaded band the 90% HDI on $q(a)$ and the dashed line the ceiling (a child cannot say more distinct words than they understand). The per-age posterior of $q(a)$ (median and HDI) is tabulated under [Production ratio](#production-ratio) above.
