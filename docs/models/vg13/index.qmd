...
::: {.callout-note}
Drafted by an LLM-based AI tool (OpenAI Codex/GPT-5; dispersion and random-effect scale priors by Claude Code/Opus 5).
:::
...
Study random-intercept scales use the shared HalfNormal(0.5) prior on the logit scale for both understood and production-ratio trajectories; the child scales use a calibrated HalfNormal(1.5), against estimates of 0.77 and 1.12 for this model (see `docs/models/PRIORS.md`). This is regularising but still allows meaningful between-dataset differences. The study effects are included in the likelihood and omitted from the reported population-level curves.
...
Because this model carries dataset and child random intercepts on both trajectories, its dispersion parameters describe what is left once a child's own level is known. The anchors are calibrated on that conditional scale: 42 and 124 for understood, 36 and 30 for the production ratio. The understood `kappa` rises across this window, which the earlier parameterisation could not represent at any setting; its `kappa_min` of 30 is correspondingly a *young*-age asymptote rather than a floor, since a rising exponential term contributes almost nothing below the young anchor and a third of the 8-18 month frame sits there. The per-age profile behind the rise is jagged — 19.6, 21.0 and 110.7 at 14, 15 and 16 months — so both understood anchors take a wider prior than the ratio ones.

The rise describes the model's `kappa` parameter, not children's comprehension. Understood counts come from forms holding 396 or 418 items but are scored out of the common 810-item reference inventory, so the modelled proportion compresses as children work up a form and the apparent between-child spread falls across this window; the child intercept's scale is fixed in age, so `kappa` absorbs the difference, and the 16-month cell is where the observed spread first drops below it. The production ratio, whose denominator is the child's own understood count and so cancels the form, shows none of this — which is why its two anchors are near-equal and take the narrower prior. See `docs/models/PRIORS.md`, "What a rising `kappa` on the understood outcomes means".
