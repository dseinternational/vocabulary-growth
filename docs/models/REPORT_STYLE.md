...
- `expected_counts_by_month_*.csv` and `posterior_summary_monthly_*.csv` are byte-identical; the `*_smoothed.csv` sidecars are byte-identical to their unsmoothed originals, so a reader downloading the "smoothed" CSV gets the unsmoothed numbers.
