...
This model examines the cross-sectional association between age and words understood in typically developing children, extending VG04 with dataset and child random intercepts to account for between-lab variation and repeated Wordbank administrations.
...
The trajectory priors match the TD understood baseline anchors at 12 and 26 months. The low anchor is strongly floor-oriented, while the high anchor allows rapid TD comprehension growth without forcing a ceiling. The dataset random-effect scale uses a HalfNormal(0.5) prior on the logit scale and the child one a HalfNormal(1.5), the latter calibrated against an estimate of 0.74 for this model (see `docs/models/PRIORS.md`). The 19-month GP anchor is an identifiability constraint rather than a substantive developmental breakpoint.
...
Dispersion follows `kappa(age) = kappa_min + exp(a_kappa + b_kappa * z)`, with the age term given priors at 12 and 20 months rather than an intercept at the pool's mean age and a slope. `a_kappa` and `b_kappa` are derived from those two anchors, so both priors are directly comparable with a per-age dispersion estimate and neither shifts meaning if the pool's age distribution changes.

Because this model carries dataset and child random intercepts, its `kappa` describes the variation left once a child's own level is known, not the total variation between children at an age. The anchors are calibrated on that conditional scale — 43 at 12 months and 66 at 20 months, against 11 and 13 for the same data with the random effects removed. The sign of `b_kappa` is unconstrained, which matters here: the fitted `kappa` *rises* with age, which the earlier parameterisation could not represent at any setting.

That rise is a property of the model's parameterisation rather than of children's comprehension, and should not be read as the latter. Comprehension is collected only on forms holding 396 or 418 items but is scored out of the common 810-item reference inventory, so the modelled proportion compresses as children work up a form; the between-child spread on the logit scale falls with age as a result, the child intercept's scale is fixed in age and cannot follow it, and `kappa` — the only age-varying spread term in the likelihood — takes up the difference. On the instrument's own scale the dispersion falls. See `docs/models/PRIORS.md`, "What a rising `kappa` on the understood outcomes means".
...
![Dispersion prior distribution `kappa_excess_old_dist` (20 months)](kappa_excess_old_dist.png){#fig-kappa-excess-old .lightbox fig-align="left"}
...
The posterior distribution of the derivative of expected understood words with respect to age, in words per month. This is a cross-sectional derivative of the fitted population trajectory, not a within-child learning rate.
