...
Because this model carries dataset and child random intercepts, its `kappa` describes the variation left once a child's own level is known, not the total variation between children at an age. The anchors are calibrated on that conditional scale — 43 at 12 months and 66 at 20 months, against 11 and 13 for the same data with the random effects removed. The sign of `b_kappa` is unconstrained, which matters here: typically-developing comprehension dispersion *rises* with age, which the earlier parameterisation could not represent. See `docs/models/PRIORS.md`.
