...
So receptive vocabulary predicts expressive vocabulary with an age-dependent slope $q(a)$ — the expected number of words spoken per word understood. The figure below reads this off directly: for children with Down syndrome at several ages, it maps words understood (x-axis) to the predicted words spoken (y-axis), with the shaded band the 90% HDI on $q(a)$ and the dashed line the ceiling (a child cannot say more distinct words than they understand). The per-age posterior of $q(a)$ (median and HDI) is tabulated under [Production ratio](#production-ratio) above.
