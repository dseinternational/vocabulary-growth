...
We jointly model words understood on the 810-word reference inventory and words spoken conditionally on the observed understood count when the two measurements are logically nested. The expected proportion of words understood, $p_U(a)$, is modelled as a smooth function of age using a linear trend plus a Gaussian process on the logit scale. The proportion of words spoken is derived as $p_S(a) = p_U(a) \cdot q(a)$, where $q(a) = \text{sigmoid}(h(a))$ is the production ratio -- the fraction of understood words that are spoken at age $a$. Since $q(a) \in (0, 1)$, this enforces $p_S \leq p_U$ by construction. Both $f_U(a)$ and $h(a)$ have their own linear trend and Gaussian process components, with HSGP approximations for scalability. When both counts are available and $S\le U$, the likelihood factorises as $P(U)P(S\mid U)$: understood uses a Beta-Binomial over 810 words and spoken uses a Beta-Binomial over the observed $U$ words with mean $q(a)$. Spoken-only records and source-data violations retain a marginal Beta-Binomial over 810 words with mean $p_U(a)q(a)$.
