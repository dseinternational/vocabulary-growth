...
class ModelConfiguration:
    ...
    """
    Prior distribution for the mean proportion of words understood at the lower slope anchor.
    """
    p_slope_hi_dist: Continuous
    """
    Prior distribution for the mean proportion of words understood at the upper slope anchor.
    """
...
def prepare_model_data(
    ...
    con.close()
    analysis_df = td_df[[x_col, y_col]].dropna()
...
def prepare_data(context: ModelFitContext):
    ...
    data, analysis_df, desc_stats = prepare_model_data(
        x_col="age", y_col="understood", max_age_months=None, n_trials=800
    )
...
def configure(context: ModelFitContext):
    """
    ...
    p_slope_low_dist = pz.Beta(alpha=1.0, beta=20)
    ...
    p_slope_hi_dist = pz.Beta(alpha=1.5, beta=1.1)
...
def prior_predictive_checks(context: ModelFitContext):
    ...
    plotting.plot_prior_samples(
        ...
        context.analysis_df["understood"],
    ...
    plotting.plot_prior_predictions(
        ...
        n_trials=context.model_data.n_trials,
        x_label="Age (months)",
        y_label="Words understood (predicted)",
...
def fit(config: str) -> ModelFitContext:
    ...
    print(
        "[bold green]Fitting Model VG01: Influence of age on words understood (A → S)[/bold green]"
    )
    print("[green]============================================================[/green]")
    print()
