...
Writing it found a second omission of the same class straight away: VG15's
Dirichlet-Multinomial concentration, which has its own prior figure and is named
on that model's page as a prior-sensitivity target, had no row either.
"""
...
def test_the_priors_table_covers_every_reported_parameter(
    ...
    engine_module = __import__(engine.module, fromlist=["render_model_graph"])
    ...
    assert reported, f"{model_key} reports no parameters at all"
