...
from vocab_growth.models import composition
...
def test_reporting_grid_does_not_change_the_observed_model(key, tmp_path, monkeypatch):
    ...
    extended = replace(
        base, ages_query=(low, low + 1, high - 1), n_plot=base.n_plot + 7
    )
    logps, covariances = [], []
    point = None
    for definition in (base, extended):
        ...
        logps.append(fixed_point_logp(model, point))
        if key != "vg10":
            continue
        ...
        covariance_point = {name: value.copy() for name, value in point.items()}
        coeff = "g_unit_u_hsgp_coeffs"
        covariance_point[coeff] = np.zeros_like(point[coeff])
        origin = fn(covariance_point)
        columns = []
        for index in range(covariance_point[coeff].size):
            covariance_point[coeff].fill(0)
            covariance_point[coeff][index] = 1.0
            columns.append(fn(covariance_point) - origin)
        design = np.stack(columns, axis=1)
        covariances.append(design @ design.T)
    np.testing.assert_allclose(logps[0], logps[1], atol=1e-9, rtol=1e-12)
    if covariances:
        np.testing.assert_allclose(
            covariances[0], covariances[1], atol=1e-12, rtol=1e-12
        )


@pytest.mark.slow
def test_produced_likelihood_is_the_conditional_four_cell_distribution(
    tmp_path, monkeypatch
):
    """Enumerate the conditional distribution and check the engine's actual alpha."""
    pi = np.array([0.6, 0.1, 0.2, 0.1])
    monkeypatch.setattr(
        composition, "composition_probabilities", lambda r, q, psi: 0 * r[:, None] + pi
    )
    captured = {}
    ...
    expressions = model.replace_rvs_by_values(
        [captured["nz_prod_cells_obs"], model["conc"]]
    )
    evaluate = model.compile_fn(
        expressions, inputs=model.value_vars, on_unused_input="ignore"
    )
    ...
    counts = np.array(
        [
            [i, j, produced - i - j]
            for i in range(produced + 1)
            for j in range(produced - i + 1)
        ]
    )
