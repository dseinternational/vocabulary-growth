...
Under every model before VG16 this share is exactly zero by construction: VG05–VG10 make `q` a function of age alone, so two same-age children necessarily convert at the same rate. `beta_lag = 0` and "share = 0%" are the same statement; the share *is* the cross-lag on a readable scale.
