...
2. **Gate 4's three recovery cells.** VG25 is a registered recovery target and the wave loop is selected automatically, so the `(beta ≠ 0, rho ≠ 0)` cell is runnable today from the prior. The other two needed a way to **set** a parameter in the truth draw rather than take what the posterior or prior offers, which the harness did not have. **Built on 2026-09-12** ([202609121113](202609121113-setting-a-parameter-in-a-recovery-truth.md)): `--set-truth beta_sign_lag=0` and `--set-truth subject_re=independent`, the second a transform rather than a number because the correlations are deterministics read off a packed Cholesky factor. Both cells run end to end on VG25. All three are runnable; none has been run.
3. **No leave-one-study-out arm** (#297 check 5). `exclude_studies` is a `BivariateModelDefinition` field, and no such arm can be registered on VG25 until `JointModelDefinition` carries one. **The cost this note first gave for that — it “would restale every VG15 and VG24 fit, which is a change to make deliberately with the refit it costs” — overstates it on both halves.** The definition restale is the avoidable kind: `BACKFILL_DEFAULTS` excuses a field whose _absence_ from an older manifest is equivalent to a stated value, and `lag_same_form_only` ([#242](https://github.com/dseinternational/vocabulary-growth/issues/242)) is the same shape — a scope field added to an already-fitted family, read through one `getattr` default that reproduces the pre-field behaviour, with the claim checked off the call site in `tests/test_fit_identity.py` rather than asserted. An `exclude_studies` entry would claim that every joint fit made before the field existed filtered no studies, which is what a frame builder with no filter in it did. And the refit is not this field's cost: the executable-code signature covers the whole package, so _any_ code change makes an existing fit unverifiable for `resume`, `sync` and `publish` — the VG25 registration itself already did that to every fit of record. What a definition difference costs that a code one does not is reach, since it is fatal at every purpose including `render` and reading a trace back; that is the restale worth avoiding, and the mechanism for avoiding it exists. So this is a small piece of work rather than a blocked one. **Done 2026-09-13:** `JointModelDefinition` carries `exclude_studies`, with the fifth `BACKFILL_DEFAULTS` entry. The claim held when measured — the VG15 and VG24 fits of record validate with the entry and fail without it, and the frame hash the three joint models share did not move — and on the current frame excluding `uk_07` takes this lag's support from 191 to 139, `ie_02` to 148. The same day the leave-one-study-out pair was registered, `no-uk07` and `no-ie02`, for the two studies carrying the most support; neither is fitted. `sign-lag-uk07-marginal` remains the nearest registered arm and answers a different question — it keeps `uk_07`'s children and moves their rows from the cross-tab branch to the marginal one.
