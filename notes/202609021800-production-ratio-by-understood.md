...
- **The comparison book's "Comprehension-matched production ratio"** (`compare_ds_td_re.py comprehension`, VG20 vs VG13) already stated the #233 reading, but tabulated N = 50/100/150 and concluded "converging by N ≈ 200" — true of the population curves and opposite to the children. `run_comprehension_matched` now also writes `ds_td_comprehension_q_observed.csv` from the two hash-verified frames through the same `observed_production_ratio_at_levels` the model pages use, and the book's table runs to N = 300 with the observed children beside the curve in each population. VG13 is capped at 18 months and its population median never reaches 250 words within support, so the TD **curve** column is empty above N = 200 — the book's curves cannot show the 0.4 agreement VG21 and VG22 show, and the earlier "converging by N ≈ 200" was not supported either (Δq is +0.09 with P ≈ 1.00 at 200). The **children** columns exist at 300 in both populations: 0.23 (TD, VG13's 8–18-month frame, 381 children) and 0.13 (DS, 97 children), against a DS curve of 0.43. The prose now says exactly that, and that a statement about children at a milestone is made from the children columns.
