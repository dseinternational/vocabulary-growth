...
> [!IMPORTANT]
> Correction added on 23 September 2026 with assistance from OpenAI Codex/GPT-6. Energy correlations suggest features to investigate but do not uniquely identify the cause of poor sampling. Low BFMI can affect estimated locations as well as interval widths. Selected stable summaries do not waive the full convergence checks. See the [statistical review corrections](202609231200-statistical-review-corrections.md). The original interpretation below remains as a dated record.
...
All three surviving changes are graph changes requiring refits of the affected models. Items 1 and 2 are implemented behind definition fields defaulting to off, so that enabling them per model is a deliberate, reviewable act and no existing model's graph changes silently. Item 3 was subsequently withdrawn as well, leaving two — see the status table's third row.
...
| 3. TD `eta` widening   | **Not adopted.** Proposed here as VG12 `eta_sigma` 0.5 → 1.0; tried during the R3 sweep the following day and **withdrawn** — the widening cost VG12 27 divergences without identifying the amplitude ([202608060900](202608060900-three-prior-conflicts.md) §3). VG12 stays at `eta_sigma = 0.5`, and §5's diagnosis stands: the reported TD GP amplitudes are still largely prior artefacts, now a known open problem (that note's §6.2) rather than a pending change. **VG13 deliberately not changed** (§5). |
