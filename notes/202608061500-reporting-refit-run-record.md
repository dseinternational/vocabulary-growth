...
- **The fixed signed peak biases the fitted curve** — withdrawn; the residual analysis was confounded by study composition. The properly adjusted answer moved the peak _earlier_, not later, so the confounded comparison pointed the wrong way.
...
VG11 clears the threshold despite the lowest repeat _rate_, which forced a correction to the mechanism — the operative quantity is the **absolute amount** of within-child replication, not the proportion of children with any.
