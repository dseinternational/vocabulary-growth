...
The degenerate compositions were checked separately, because the engine can reach them: a pool where every child is seen once, which `one_observation_per_subject` produces; one where every child is seen twice; and a single row. All three build with a finite log density and gradient, and nutpie samples the first — its repeat-effect vector is legally empty, shape (0,), and the whole likelihood is quadrature. None of them needs a guard.
