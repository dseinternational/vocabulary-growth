...
**And VG15's Dirichlet-Multinomial concentration had no row either.** Not in the issue; found immediately by the coverage check written for VG22. `log_conc` has its own prior figure on the VG15 page and is named there as a prior-sensitivity target, and the table beside it did not mention it. Same class, different model, noticed by nobody — which is the argument for a contract over a corrected list.
