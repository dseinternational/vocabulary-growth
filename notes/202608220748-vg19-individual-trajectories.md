...
> [!IMPORTANT]
> Interpretation of the fitted VG19 `rep` trace of 2026-08-21, written 2026-08-22. **VG19 is not the model of record**, so nothing here is publishable yet; G3 has since returned and §6.5 records what it does and does not clear. Everything is a deterministic function of six fitted scalars propagated over posterior draws; reproduced by `scripts/experiments/vg19_individual_trajectories.py`. The headline: comprehension trajectories run close to parallel, production-ratio trajectories genuinely diverge, and that dissociation is the substantive content of the child slope.
...
5. **G3 has returned, and it qualifies §3 rather than clearing it.** [202608212000](202608212000-vg19-gates-g2-g4-g5.md) §4d: all three `rep` replicates are assessable, VG19's coverage is the best in the project (0.850 at the 89% ETI, 0.483 at the 50%, against nominal 0.89 and 0.50), and `tau_subj_u_1` and both `rho01` recover in 3 of 3. But `tau_subj_q_1` -- the parameter carrying §3 -- misses in 2 of 3, **downward** both times, with `tau_subj_q_0` biased upward alongside it. The direction is the saving grace: the engine under-recovers the rate and over-recovers the offset, so the fan reported in §3 is compressed rather than manufactured, and 0.640 is if anything an under-estimate. Treat the _existence and direction_ of the divergence as established and its _magnitude_ as a lower bound.
6. **The tables here run to 84 months, above the comprehension reporting cap of 72.** That is deliberate and permissible — this is an internal analysis note, and §3's whole subject is what happens at ages where the child structure dominates. It is also the evidence that set the cap: see [202608221200](202608221200-reporting-source-by-quantity.md) §4. Nothing above 72 months in this note may be quoted in the report.
7. **These are model-based random effects, not observed trajectories.** Shrinkage means the spread of fitted per-child effects is narrower than the spread implied here, which is the population distribution the effects are drawn from.
