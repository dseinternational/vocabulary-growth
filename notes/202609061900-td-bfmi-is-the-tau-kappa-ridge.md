...
> [!WARNING]
> **Corrected 2026-09-07 ([202609072200](202609072200-vg08-replication-thinning.md)).** The control table's VG08 figure, corr(τ_subj_u, κ_young_u) = −0.001, is not κ at the young anchor. VG08 is the only model in that table with a child effect whose κ block is the `kappa_min + exp(a + b·z)` form (VG05, VG07 and VG08 carry it; every other registered model uses the two-anchor form) and so emits no `kappa_young_u` deterministic; −0.001 is what the old-age floor `kappa_min_u` gives (a same-machine refit of the same frame gives +0.012 for the floor). Read at the 24-month young anchor off the stored `kappa_u_query` grid, VG08's ridge is **+0.384**, with BFMI 0.424 and corr(τ, energy) −0.627 reproducing the figures above. The section's headline — that the τ–κ ridge is specific to the typically developing models — is therefore **withdrawn**: VG08 carries it, weaker than the TD +0.57 to +0.76 and above the BFMI threshold, but present. What survives is that the energy cost of a child effect is general, and — now measured directly rather than inferred from two populations — that the ridge grows with the share of child effects that have no replication behind them: thinning VG08's repeaters to VG12's 17.2% on the same 943 children takes its BFMI to 0.267 and its ridge to +0.611. The replication test this note asked for has been run; its reading and what it implies for the 4.6 remedy are in the 2026-09-07 note.
