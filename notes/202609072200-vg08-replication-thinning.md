...
> [!IMPORTANT]
> **Refined the same evening ([202609072230](202609072230-vg12-repeaters-prior.md)).** Option 1 below was tested on VG12 at `test` tier. As a _prior_ it does nothing: the full frame's own posterior on the share (sd 0.028) is already tighter than the repeaters' (sd 0.035), so a prior at the repeaters' strength is outweighed by the singletons' functional-form identification, and the BFMI stays at 0.24 with the ridge at +0.69. Pinning the split at the repeaters' value — a cut — takes the BFMI to 0.589 and removes the ridge (−0.16). The recommendation stands with that correction: option 2 as a cut with the repeaters' uncertainty propagated, not as a prior. The bivariate TD models, which have no partition, are untested.
