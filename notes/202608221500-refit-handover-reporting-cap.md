...
- **VG21 and VG22 are registered** (`9994eab`). `fit_model.py all` derives its list from `MODEL_REGISTRY`, so both are picked up automatically; the three agent-instruction files were corrected on 2026-08-25.
- **The runbook's parallel-fitting recipe was a second stale copy of the model set, corrected 2026-08-26.** The claim above that the agent-instruction files were the _only_ place listing the old set was wrong. `docs/runbooks/full-refit.md` §"Parallel fitting on a large VM" splits the registry by hand into a DS pool and a serial TD pass and passes each as an explicit `-Models` list, which bypasses the driver's registry-derived default: VG22 was in neither list, and VG21 and VG23 were in neither. **Nothing would have reported it.** `run_replication.ps1` validates only the models it was given, so the two passes would have fitted 17 of 20 and left `SUCCESS`; the omission surfaces only at the final `check_fit.py all`, after the fitting window has been spent. `tests/test_runbook_model_lists.py` now parses the lists out of the runbook and checks them against `MODEL_REGISTRY` — parsed rather than restated, so there is no second copy to keep in sync — and it was confirmed to go red on the pre-fix document. This is the third hand-copied model list to drift from the registry, after the agent-instruction files and the sensitivity suite's base-model map.
