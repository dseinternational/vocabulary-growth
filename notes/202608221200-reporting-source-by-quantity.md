...
## 8. Rerun on the enlarged pool, 2026-09-09

The test §4 set — has the VG19–VG20 gap fallen to the 0.15 that holds below 60 months? — was rerun on the 2026-09-08 `rep` fits after the `us_03` ingestion, by the same script. **It has not.** `q` at 72 months still differs by **0.89** of VG20's interval width (VG19 0.87 against VG20 0.77), the spoken population curve by 0.58 at 72 and 0.59 at 84, and the understood curve by 0.12; below 60 months nothing exceeds 0.15 on the two most-reported curves. The 72–84-month band gained no comprehension observations — 23 understood rows across 61 children, against 25 before — because `us_03` is a younger cohort. With the cap at 72 the summaries stop there, so the check is at 72; 84 is outside the reported grid. **The cap stays at 72.** Recorded in [`202609091700`](202609091700-comprehension-cap-check-rerun.md). VG19 became a development step the same day, so the next rerun needs an explicit `-Models vg19,vg20` refit rather than a cycle's by-product.
