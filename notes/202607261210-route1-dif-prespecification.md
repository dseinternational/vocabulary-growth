...
7. **Duplicated-outcome sensitivity, and a direct test of the mechanism.** Refit with the six excluded WG administrations reinstated (`include_duplicated_outcomes=True`), and report `r` both ways: if the verdict turns on six records, that is a result about fragility and must be stated as one. Separately, the item-level pull affords a decisive check the aggregate data cannot give — a duplicated column appears as two _identical_ response vectors. Report, for each of the six, whether the comprehension and production vectors are identical, near-identical, or unrelated, and record the outcome as confirming or refuting the signature. A refutation reinstates them and is logged in §10.
