...
> Clarification added to the deviations log with assistance from OpenAI Codex/GPT-6.

> [!WARNING]
> Pre-specification, 2026-07-26, committed **before** any item-level data enters the repository — the git history is the timestamp. It binds the Route 1 analysis described in [`202607261540-item-difficulty-and-the-aggregate-likelihood.md`](202607261540-item-difficulty-and-the-aggregate-likelihood.md) §11 once execution begins. Its two **[owner decision]** items were settled by the study owner on 2026-09-14 — `r* = 0.30` confirmed, and production kept as the primary outcome subject to a design-analysis rule fixed in §7 — and the same day's pre-execution record in §10 notes what has changed about the data since this was written. Deviations after execution begins are permitted but must be logged in this note's §10 with their reason, and the primary estimand, threshold and decision rule may not be changed after any Down syndrome item-level response has been seen.
...
Everything else the companion notes raise — the `kappa` interpretation in particular — is a bounded-scale and link issue that this analysis neither tests nor repairs.
...
**Eight WG administrations are excluded before fitting**, under the duplicated-outcome rule established in [`202607261245-edgin-duplicated-outcome-records.md`](202607261245-edgin-duplicated-outcome-records.md) and implemented in `data_utils.mask_duplicated_outcome_administrations`: production at or above 0.75 of comprehension at 11–18 months, which is a data-preparation artefact rather than exceptional ability on three independent lines of evidence. This exclusion is **not** outcome-selection of the kind §5 forbids — it is a pre-specified validity criterion fixed before any item-level response is seen, its false-positive rate is measured (0.69% in a 2,480-administration typically-developing reference), and §8.7 both re-runs the analysis with the records reinstated and verifies the mechanism directly once item-level vectors exist. Two further rules apply to the same subset — one duplicate administration is de-duplicated, and 30 production counts matching a near-ceiling or longitudinal-collapse signature are masked (§8.4 reinstates those as a sensitivity).

Counted through the loaders rather than by hand, the Route 1 sample is therefore **78 WG administrations from 50 children** for comprehension — three children (81114, 81122, 81132) have a single WG record each, all flagged, and so leave the comprehension analysis entirely — and **165 administrations from 102 children** for production. Beyond these documented rules, no administration may be excluded on the basis of its responses.

> [!IMPORTANT]
> **Superseded counts, 2026-09-14.** Everything in this section describes the Wordbank by-child export, which `us_01` stopped being on 2026-08-03: it is now derived from the Edgin item-level contributor files, and three exclusion rules have been added since. Counted through the loader today, the sample is **126 comprehension administrations from 57 children and 209 production administrations from 57 children** — the 102 below counted each form's children separately, because the export issued a new child id per form. See §10's pre-execution record.

> [!NOTE]
> An earlier draft gave 81 comprehension administrations from 50 children and 190 production administrations from 116 children. Those figures predated the widening of the ratio threshold to 0.75 and the production-signature rule, and were derived by hand; the ones above come from `load_combined_data` itself, so they match what a fit would actually see.
...
**Decided 2026-09-14 by the study owner: production stays primary provisionally, and the design analysis chooses between the outcomes by a rule fixed now, so nobody chooses after seeing its results.** The layout makes the question live rather than hypothetical: on the current frame production is the more floor-bound outcome (41 of 209 administrations at zero, median 5 words) and comprehension the less (2 of 126, median 60), from the same 57 children (§10). The rule:

1. For each outcome, take from the §8.2 simulations the probability that the §5 decision rule returns the **correct conclusive verdict** — _order-equivalent_ at true `r` = 0 and at 0.1, _material DIF_ at true `r` = 0.5 — and average the three.
2. **Comprehension becomes primary if its average exceeds production's by more than 0.10.** Otherwise production stays primary.
3. The grid points `r` = 0.2 and 0.3 sit at or beside the threshold, where no verdict is meant to be conclusive; they are reported and do not enter the comparison.
4. Replicates are run until the Monte Carlo standard error of the difference between the two averages is below 0.03.
5. The rule is applied once, on the design analysis alone, before any Down syndrome item-level response is examined. The outcome it does not select is reported as the secondary, under the same threshold and decision rule.

The owner's decision is the option — a pre-committed rule rather than a fixed designation. The rule's details above (the grid points, the 0.10 margin, the precision requirement) were drafted by the analyst when the decision was recorded; it may be amended before the design analysis runs and not after.
...
6. **Person-trend sensitivity.** Replace the quadratic `mu_g` with a spline; `r` should be insensitive if the person side is truly nuisance.
   6b. **Discrimination sensitivity (2PL), and why it is not optional.** The model in §4 is 1PL: equal discrimination across items. That assumption is what buys Rasch sufficiency, and it is the reason the aggregate likelihood is safe — but it is a real threat _here_, because a 1PL fitted to items whose discriminations genuinely differ can manifest as differential item functioning that is not there. The two populations occupy different ability ranges (the whole point of "later"), and an item with atypical discrimination will appear easy in one range and hard in the other, which is exactly the signature `delta_j` is built to detect. Refit with a 2PL — item discriminations `a_j ~ LogNormal(0, 0.3)`, difficulties and DIF as before — and report `r` both ways. If the 1PL and 2PL verdicts disagree, the 2PL governs and the disagreement is the finding, because the 1PL's apparent DIF cannot then be distinguished from discrimination heterogeneity. Note this cuts against the primary specification rather than for it, which is why it is registered here rather than left to discretion.
...
1. Owner settles the two **[owner decision]** items: the threshold `r*`, and primary-outcome designation (production, as drafted, or revised per the design analysis). _Done 2026-09-14 — see the pre-execution record below._
...
_Pre-execution record, 2026-09-14. Execution has not begun, so none of these is a deviation; they are logged so the state of the data at the moment the owner decisions were taken is on the record._

1. **Owner decisions.** `r* = 0.30` confirmed unchanged (§5). Primary outcome: production, subject to the design-analysis rule in §7 (option (c) of the three put to the owner; (a) was production fixed, (b) comprehension fixed).
2. **The Down syndrome sample has changed since §3 was written.** On 2026-08-03 `us_01` was rebuilt from the Edgin item-level contributor files in `langcog/wordbank` (`scripts/build_us01_source.py`; `data/vocab_data_us_01.md`), because the by-child export age-truncated the administrations and issued a separate child id per form. Counted through `load_combined_data()` on 2026-09-14: **comprehension 126 administrations from 57 children** (2 at zero, median 60 words) and **production 209 administrations from 57 children** (41 at zero, median 5), ages 11–27 months; 53 children took both forms and 28 have five administrations. §3's 78 / 50 and 165 / 102 are superseded, and §9's concern that `u_i` and `e_it` are separated mainly by one close WG-then-WS pairing is weaker than it was. The design analysis uses the current layout, as §8.2 requires.
3. **Three exclusion rules added since §3 reach `us_01`.** Each was fixed on aggregate counts, before any item-level response was examined, and each has a reinstatement flag: administrations below their form's lowest registered age (`include_below_form_floor`, 2026-08-03), children recorded only at their form's ceiling (`include_ceiling_only_children`, 2026-08-03), and production counts contradicted by a same-day administration on another form (`include_same_day_disagreements`, 2026-08-31). §3's "no administration may be excluded on the basis of its responses" is read as applying beyond these documented rules, as it already did for the rules it names.
4. **§8.7's open question is answered: an item-level source exists.** The contributor files hold every Edgin administration's word-item responses. `scripts/build_us01_source.py` reads them, and uses them only to sum word items per administration and to identify administrations with every item blank. The repository holds no item-level file, and no committed code computes a per-item proportion, a difficulty or a response pattern for either population. The freeze condition in the header — any Down syndrome item-level response _seen_ — is therefore taken as not yet met, and the owner decisions are recorded before that changes.
...
_Methodological clarification, 2026-09-23, with assistance from OpenAI Codex/GPT-6._ This review changes no registered estimand, threshold, decision rule or owner decision. The sufficiency statement in §1 conditions on the item parameters. It does not establish that totals contain no information about item difficulty, or that heterogeneous difficulties have no effect on an aggregate likelihood. The earlier numerical approximation is specific to its simulation settings. The ceiling-only and same-day disagreement exclusions in the 14 September record use observed outcomes, even though source concerns motivate them. Their timing does not make them outcome-independent. The planned reinstatement and item-level sensitivities remain necessary. See the [statistical review corrections](202609231200-statistical-review-corrections.md).
