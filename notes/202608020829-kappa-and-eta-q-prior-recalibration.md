...
> [!CAUTION]
> **§22's Down syndrome joint prior values were superseded on 2026-08-19** (`02bef31`, [#229](https://github.com/dseinternational/vocabulary-growth/issues/229)). `_DS_JOINT_UNDERSTOOD_KAPPA_RE` and `_DS_JOINT_Q_KAPPA_RE` now anchor at **(18, 72)** rather than (24, 48), so the whole reporting range is interpolation rather than extrapolation onto the asymptote, and `kappa_min_mu` moves from `log(3.0)` to `log(7.8)` where a conditional calibration at the new anchors puts it. Excess medians are the calibrated totals at the new anchors minus the floor prior's median: 84.8 and 6.2 on understood, 10.4 and 0.6 on `q`, the last at `sigma` 1.5 rather than 1.0 because its calibration log-scale SE is 1.429.
>
> **The replacement drops this section's a priori bias correction.** §22 set its medians as each estimate divided by the recovery bias measured at that level (81.6/0.74, 20.3/0.62, 13.8/0.83, 7.6/0.70); the 2026-08-19 values are uncorrected. That was not a considered choice at the time — it was noticed afterwards — and the reasoning for living with it is in [`202608191800`](202608191800-kappa-components-not-estimands.md) §9, which also credits §22 for the components-versus-totals finding it had independently re-derived.
>
> §22's _reasoning_ stands unchanged and is still the best statement of it in the repository: the estimates are lower bounds because the error is one-directional and monotone in the level; and only the sum at the anchors is identified, the floor and the excess trading off against each other. The 2026-08-19 measurements agree, adding that the floor's share of reported `kappa` at 84 months has an 89% interval of [13.2%, 78.3%] on understood and [21.0%, 99.8%] on `q`, and that a 160% move in the floor's prior shifts reported `kappa` by 7-14% at 72-84 months and under 1% below 48.
