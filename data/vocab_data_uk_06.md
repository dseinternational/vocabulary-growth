...
The **standard DSE vocabulary checklists** — Checklists 1, 2 and 3, listing 120, 340 and 350 words, so **810 items** in total. This is the same instrument as `ie_01` and `ie_02`, and it is the common reference inventory every model's likelihood scores against (`n_trials = 810`), so `survey_vocab_max = 810` is the source's native ceiling rather than a harmonisation.
...
- **`imitated` is internally inconsistent, and is not used.** Column 3 is "understands and imitates", so it must be nested within `understood` — but it exceeds it on **4 of 11** rows, and on one row reaches **822**, above the instrument's own 810-item ceiling. The `vocab_combined` view selects only `understood`, `spoken`, `signed` and `produced`, so this column reaches no model and affects no estimate. It is recorded here because it is a live data-quality question for the source: a count above the form ceiling suggests a transcription or column-alignment problem that could in principle have touched neighbouring columns, even though `signed` and `spoken` are individually consistent on every row.
