...
This is the only source in this repository that carries a **typically developing comparison group**. Its non-vocal expressive modality is described by the source as _gestural_, and it is a **lexical** one — gestures representing specific lexical items, each tied to one of the 651 checklist words and scored per word, which is the same coding `uk_02`, `uk_07` and `nz_01` apply to signs. What that coding marks is the child's manual lexicon — gestures "properly taught or spontaneously learnt", which the source does not separate and whose authors use _signed_ and _gestured_ interchangeably; the typically developing group's totals (23.6 gestured words on average, with no sign instruction reported) show that spontaneous symbolic gesture is captured. Whether the other signing sources' "signs" columns were ticked more narrowly is not recorded anywhere, so no construct distinction can be drawn on either side. It is read as this repository's `signed` column (see [Measurement and column semantics](#measurement-and-column-semantics)) on the same footing as the other sources, with one measurement caveat — the eleven-category elimination under Known issues. See [`notes/202609021903`](../notes/202609021903-es01-gesture-construct-revisited.md), which supersedes [`notes/202608271551`](../notes/202608271551-es01-gesture-construct.md).
...
Second, **`spoken_or_gestured` is a union, not an intersection.** "WORD PRODUCED + GESTURES ONLY" could be misread as "words produced by word *and* gesture only". The data rule that out: the column exceeds `min(spoken, gestured)` on all 186 Down syndrome rows, and is at least `spoken` on 186 of 186.
...
The `vocab_combined` view admits the **Down syndrome children only** (`group = 'DS'`, 186 rows). The typically developing children are a Spanish-normed comparison sample on a different instrument from the Wordbank forms the TD reference pool draws on, so pooling them would change the defined Wordbank reference sample. The matched design is analysed separately. They remain in the `vocab_es_01` table for a matched-pair analysis.

`gestured` becomes the view's **`signed`** column and `spoken_or_gestured` becomes **`produced`**. The symbolic gestures are a non-vocal expressive lexicon scored per word — the same **coding** the signing models (VG14, VG15 and the registered descendants of VG15) consume; its study terms (`delta_sign`, `delta_psi`) allow systematic level differences. They do not establish measurement equivalence or remove every effect of pooling ([`notes/202608281147`](../notes/202608281147-study-term-pooling-licence.md)). Like `uk_02` and `nz_01`, and unlike `uk_01` (see `SIGNED_ONLY_STUDIES`), the count is a **total**, covering words gestured whether or not they are also spoken, so it needs no item-level re-derivation to enter the pool on the same total-count footing. This makes es_01 the second-largest signing source in the pool at 185 usable signed observations, and its `produced` is a de-duplicated union recorded by the source itself rather than reconstructed (see the `model_vg18` docstring for how `produced` varies across sources). `produced` exceeds `spoken` by a mean of 28 words.
...
Two caveats. Mantel-Haenszel is a crude descriptive statistic on the observed cells, not $\psi$ itself, which is population-conditioned against the fitted $r$ and $q$. And `nz_01` has no comprehension total, so its "neither" cell spans all unproduced items rather than understood-but-unproduced, which inflates its odds ratio — the same data for `uk_07` reads 13.90 within understood and 40.72 over all 674 items. Both the magnitude and direction of an odds ratio depend on the reference set and its "neither" cell. The share of gestured words also spoken omits that cell, but describes overlap rather than association relative to independence. Rows in this audit are administrations; repeated visits from one child are not independent children.

What survives every control is that es_01 sits at independence while the three sign sources are positive. By age band it runs 0.30–1.12 against 4.4–41.6 for `uk_02` and 4.4–18.1 for `uk_07`, with no overlap in any band; matched on expressive vocabulary (30–300 words) it is 1.05 against 4.80 and 9.68. On the conditioning-free share-also-spoken measure it is the low end of a continuous gradient rather than categorically apart. Why is not established ([`notes/202609021903`](../notes/202609021903-es01-gesture-construct-revisited.md)). Three candidates of unknown size stand: what parents counted (es_01's column is defined to include spontaneous gestures alongside taught signs, while the other sources' sign columns carry no recorded definition); the eleven-category elimination under Known issues, which — if the totals carry it — records a word signed and said in an eliminated category as spoken-only and so deflates the odds ratio toward independence; and signing instruction, which no source records. Four studies cannot separate them, and the spread among the three sign sources is unexplained on the same terms.
...
$\psi$ now carries `delta_psi`, a zero-sum study random intercept over the $\psi$-informed studies, with `tau_psi` quantifying the spread. Each source has its own partially pooled association. The common centre still depends on the included studies, so adding a source can move it. The flag defaults to `True`; the per-study estimates and exclusion sensitivity show the contribution of this source. Setting it `False` isolates es_01's contribution. The recorded analysis found no clear age contribution after accounting for study: see [202608121030](../notes/202608121030-psi-heterogeneity-and-age-invariance.md).
...
- **Age scale.** The source gives age both as months-and-days and as a day total, related by exactly 30 days per month, so one is a rendering of the other rather than an independent measurement. `age` and `age_days` follow the document's months-and-days column, which is plausible on every row; the day total contains one impossible value. If the day total is instead the true count, the ages here run about 1.4% high (≈0.5 months at the DS mean of 34). Taking the document at its word puts the TD group's mean developmental quotient at 102.5 against 104.0 for the alternative reading — mildly in its favour, but not decisive. Eight rows disagree between the two columns; five shift the age under the earlier rounding rule by 1–3 months.
- **Eleven categories eliminated from the gesture data (pending author confirmation).** Galeote et al. (2011) report a validity exercise on 66 parents who were asked to describe each of their child's gestures; categories whose gesture reports "frequently presented problems" were identified — words for people, body parts, food and drink, clothes, objects and places at home, objects and places away from home, questions, prepositions, auxiliary verbs, periphrasis, and sentence connectors — and the authors "decided to be conservative and eliminate these categories from the data". That is 11 of the instrument's 21 categories, leaving the gesture assessment spanning 10. The elimination can only have applied to the gesture column: two DS children reach the full 651 on comprehension, so the complete checklist stands behind `understood` (and `spoken`), while the DS `gestured` maximum is 214. Our totals reproduce the paper's Table 2 group means exactly, so whatever treatment the published analysis carries, this file carries — whether that includes the elimination is a question for the author. If it does, `gestured` (and the cells derived from it) is measured over a substantially reduced item universe relative to `understood` and `spoken` — plausibly around half — and items in eliminated categories can land only in the spoken-only or neither cells: a word signed and said becomes spoken-only, a word gestured only becomes neither. The eliminated categories include the early concrete nouns (people, body parts, food and drink, clothes, household objects) where sign-and-say overlap concentrates, so the likely net effect is to deflate the within-understood odds ratio, by an amount that the eliminated share alone does not bound — a worked example in the note reaches a factor of 2.7 with modest concentration. It is checkable by simulating the elimination on `uk_07`'s item-level deposit, and settled only by the author. See [`notes/202609021903 §4`](../notes/202609021903-es01-gesture-construct-revisited.md).
