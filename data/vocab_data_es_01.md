...
`gestured` becomes the view's **`signed`** column and `spoken_or_gestured` becomes **`produced`**. The symbolic gestures are a non-vocal expressive lexicon scored per word — the same **coding** the signing models (VG14, VG15, VG17) consume; whatever differences in what was counted exist between es_01 and the sign sources (see above) are carried on es_01's own study terms (`delta_sign`, `delta_psi`) rather than averaged away, which is what makes the pooling defensible ([`notes/202608281147`](../notes/202608281147-study-term-pooling-licence.md)). Like `uk_02` and `nz_01`, and unlike `uk_01` (see `SIGNED_ONLY_STUDIES`), the count is a **total**, covering words gestured whether or not they are also spoken, so it needs no item-level re-derivation to enter the pool on the same total-count footing. This makes es_01 the second-largest signing source in the pool at 185 usable signed observations, and its `produced` is a de-duplicated union recorded by the source itself rather than reconstructed (see the `model_vg18` docstring for how `produced` varies across sources). `produced` exceeds `spoken` by a mean of 28 words.
