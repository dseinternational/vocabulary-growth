...
This is the only source in this repository that carries a **typically developing comparison group**. Its non-vocal expressive modality is described by the source as _gestural_, and it is a **lexical** one — gestures representing specific lexical items, each tied to one of the 651 checklist words and scored per word, which is the same coding `uk_02`, `uk_07` and `nz_01` apply to signs. What that coding marks here, though, is the child's **symbolic-gesture repertoire** — gestures "properly taught or spontaneously learnt", undifferentiated — a related but broader construct than the taught lexical signing the UK and New Zealand sources record, as the typically developing group's own totals show: 23.6 gestured words on average, with no sign instruction reported and none plausible at scale. It is read as this repository's `signed` column (see [Measurement and column semantics](#measurement-and-column-semantics)) with that construct caveat: the coding is shared, the construct is not. See [`notes/202608271551`](../notes/202608271551-es01-gesture-construct.md).
...
What survives every control is that es_01 sits at independence while the three sign sources are positive. By age band it runs 0.30–1.12 against 4.4–41.6 for `uk_02` and 4.4–18.1 for `uk_07`, with no overlap in any band; matched on expressive vocabulary (30–300 words) it is 1.05 against 4.80 and 9.68. On the conditioning-free share-also-spoken measure it is the low end of a continuous gradient rather than categorically apart. The construct re-examination of [`notes/202608271551`](../notes/202608271551-es01-gesture-construct.md) supplies the most economical first-order reading: a spontaneous gesture repertoire is deployed for words the child cannot yet say, where taught key-word signing is sign-and-say by construction, so es_01's independence is expected under the gesture construct rather than anomalous. The residual spread among the three sign sources remains unexplained — signing instruction is the obvious candidate, but no source records it, and four studies cannot test it.
