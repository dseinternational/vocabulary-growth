...
Extends `scripts/loso_compare.py`:
...
- `output/comparisons/loso_loo_v2_<MODEL>.csv` — per-model LOSO summary.
- `output/comparisons/loso_compare_v2_conditional.csv`
- `output/comparisons/loso_compare_v2_marginal.csv`
- `output/comparisons/loso_compare_v2_summary.csv` — combined table.
"""
...
def main() -> None:
    ...
    for spec in SPECS:
        ...
        ).to_csv(os.path.join(OUT_DIR, f"loso_loo_v2_{spec.short}.csv"), index=False)
    ...
    summary_df.to_csv(
        os.path.join(OUT_DIR, "loso_compare_v2_summary.csv"), index=False
    )
    print("\nSummary:")
